...
class WordListParserKeepBefore(object):
    '''
    Extracts a substring from the beginning to a given string from
    a word in a list (not included in the selection). The whole string if not found
    '''
    def __init__(self, cm, list_file_name):
        ...
        self.patternList = [re.compile(r"(.*?)\s*"+w.replace("\n","")+r".*", flags=re.I) for w in wordList] # builds regexps for the word list
    ...
    def cleanValue(self, v):
        ...
        if v is not None:
            ...
            # returns a result if at least one match is found
            if len(matches)>=1:
